...
/**
 * Official OpenRouter brandmark, exact geometry from the supplied SVG.
 *
 * Two exports so the same asset works in both places on the callout:
 *  - OpenRouterMark   : the routing glyph alone, in `currentColor` (white in the CTA).
 *  - OpenRouterLockup : the full mark + "OpenRouter" wordmark, two-tone (blue mark,
 *                       white wordmark) to match the header treatment.
 */

// The routing glyph (converging arrows). No own fill/stroke — inherits from parent
// so it can be recoloured per placement.
const MARK_PATHS = (
  <>
...
/** Full lockup: brand-blue mark + white wordmark, sized by height. */
export const OpenRouterLockup: React.FC<{ className?: string }> = ({ className = 'h-6 w-auto' }) => (
  <svg
    viewBox="18 110 1174 182"
    className={className}
    role="img"
    aria-label="OpenRouter"
    xmlns="http://www.w3.org/2000/svg"
  >
    <path className="text-white" fill="currentColor" d={WORDMARK_D} />
    <g className="text-[#5b7cfa]" fill="currentColor" stroke="currentColor">
      {MARK_PATHS}
    </g>
  </svg>
);

export default OpenRouterMark;
